...
describe("TodoItemCheckbox Component", () => {
    ...
    test("renders as not checked when completed is false", () => {
        const { getByTestId } = render(
    ...
    test("calls onToggle with the correct todoId when clicked", () => {
        const { getByTestId } = render(
            <TodoItemCheckbox todoId={4} completed={false} onToggle={onToggleMock} />
        );
        const checkbox = getByTestId("todo-item-checkbox");
    ...
    test("toggles state correctly on change", () => {
        ...
        expect(checkbox).not.toBeChecked();
        fireEvent.click(checkbox);
        expect(onToggleMock).toHaveBeenCalledWith(5);
    ...
    test("renders with the toggle class", () => {
        const { getByTestId } = render(
            <TodoItemCheckbox todoId={6} completed={false} onToggle={onToggleMock} />
        );
        const checkbox = getByTestId("todo-item-checkbox");
        expect(checkbox).toHaveClass("toggle");
    });
