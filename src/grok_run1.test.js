...
describe('EditTodoInput', () => {
  ...
  test('renders the input field with default value', () => {
  ...
  test('does not call onBlur when onBlur prop is not provided', () => {
    ...
    const input = screen.getByTestId('edit-todo-input');
    fireEvent.blur(input);
    // No error should be thrown, and no call is expected since onBlur is not provided
  });

  test('sanitizes special characters correctly before saving', () => {
    render(<EditTodoInput onSaveEdit={mockOnSaveEdit} />);
    const input = screen.getByTestId('edit-todo-input');
