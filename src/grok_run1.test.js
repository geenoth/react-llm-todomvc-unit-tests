...
describe('TodoItemCheckbox', () => {
  ...
  test('updates aria-label based on completed state', () => {
    render(<TodoItemCheckbox {...defaultProps} completed={false} />);
    let checkbox = screen.getByTestId('todo-item-checkbox');
    expect(checkbox).toHaveAttribute('aria-label', 'Mark todo 1 as complete');

    render(<TodoItemCheckbox {...defaultProps} completed={true} />);
    checkbox = screen.getByTestId('todo-item-checkbox');
    expect(checkbox).toHaveAttribute('aria-label', 'Mark todo 1 as incomplete');
  });
