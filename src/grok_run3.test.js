...
describe('TodoLabel Component', () => {
  ...
  test('renders the label with the correct title', () => {
  ...
  test('calls onEdit on Enter key press for accessibility', () => {
    render(<TodoLabel {...defaultProps} />);
    const label = screen.getByTestId('todo-label');
  ...
  test('calls onEdit on Space key press for accessibility', () => {
    render(<TodoLabel {...defaultProps} />);
    const label = screen.getByTestId('todo-label');
  ...
  test('does not call onEdit on other key presses', () => {
    render(<TodoLabel {...defaultProps} />);
    const label = screen.getByTestId('todo-label');
    fireEvent.keyDown(label, { key: 'A', code: 'KeyA' });
    expect(mockOnEdit).not.toHaveBeenCalled();
  });
});