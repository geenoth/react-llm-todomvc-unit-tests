...
describe('ToggleAllCheckbox component', () => {
  it('renders correctly', () => {
    ...
    expect(getByTestId('toggle-all-checkbox')).toBeEnabled();
  });
  ...
  it('calls onChange callback with correct value on click', () => {
    ...
    const { getByTestId } = render(<ToggleAllCheckbox onToggleAll={onChangeMock} />);
  ...
  it('toggles checkbox on click', () => {
    const { getByTestId } = render(<ToggleAllCheckbox allCompleted={false} />);
    const checkbox = getByTestId('toggle-all-checkbox');
    fireEvent.click(checkbox);
