...
describe('TodoItemCheckbox', () => {
  ...
  it('renders checkbox with correct aria-label when completed is false', () => {
    const { getByTestId } = render(<TodoItemCheckbox onToggle={onToggle} todoId={todoId} />);
    expect(getByTestId('todo-item-checkbox')).toHaveAttribute('aria-label', `Mark todo ${todoId} as complete`);
  });

  it('renders checkbox with correct aria-label when completed is true', () => {
